...
## Passo 4: Entregar a calculadora interativa e os gráficos (o "efeito uau")
...
Além da calculadora, o HTML entrega três gráficos, sempre em SVG/CSS puro, sem biblioteca externa, mesmos tokens de cor do resto do documento. Tabela crua não é suficiente pra este passo, gráfico é obrigatório, não opcional:

- **Gráfico de conversão por etapa, com linha de referência:** barras verticais, uma por par de etapas consecutivas, altura proporcional à conversão calculada no Passo 3. Trace uma linha tracejada horizontal na altura da média saudável (a mesma média já calculada no Passo 3). Pinte de vermelho (ou cor de alerta equivalente aos tokens do documento) qualquer barra que estoure a média pra cima (sinal de avanço em lote) ou fique muito abaixo dela (o gargalo real). É o gráfico de maior ganho de leitura sobre a tabela: torna o desvio óbvio sem o usuário comparar número por número. **Se mais de um par de etapas passar de 100%** (pode acontecer por motivos diferentes: avanço em lote real, ou o próprio jeito de calcular estoque contra uma etapa terminal cumulativa), trate cada um do jeito que fizer sentido pro seu caso, mas nunca deixe implícito que existem dois achados iguais, documente a distinção entre eles em uma frase cada. **Barra acima de 100%:** capar a altura visual no teto do gráfico (mesma altura da barra de 100%), aplicar um padrão hachurado nela pra sinalizar visualmente que é um valor fora de escala, e mostrar o valor exato (ex.: "183%") no rótulo da barra, nunca deixar a barra estourar o gráfico nem distorcer a escala das outras.
- **Funil visual:** barras horizontais, uma por etapa do funil, largura proporcional à quantidade real de negócios ali. Não force as barras a ficarem decrescentes: se o dado real mostrar uma etapa mais cheia que a anterior (sintoma do próprio gargalo, avanço em lote empilhando negócio ali), mantenha a largura fiel ao número real e registre isso como parte do achado visual, mascarar esse padrão pra manter a barra "bonita" esconderia exatamente o problema que o gráfico existe pra mostrar. Destaque em vermelho a etapa gargalo e, se a causa real estiver numa etapa anterior (achado do Passo 3), destaque as duas, com uma nota curta explicando por que as duas aparecem destacadas juntas. **Se a etapa gargalo e a etapa causa real forem a mesma** (resultado válido, já coberto no Passo 3), aplique um único destaque nessa etapa, não dois destaques sobrepostos.
- **Comparação valor represado vs receita recuperável:** duas barras simples lado a lado, uma com o valor represado hoje (Passo 5.1) e outra com a receita adicional recuperável (também Passo 5.1). É o gráfico mais barato de fazer, os dois números já estão calculados, só precisa virar barra.

Todos os três usam sempre dado real calculado nos passos anteriores, nunca número de exemplo. Se algum dado necessário não existir (ex.: menos de duas etapas com conversão válida), omita o gráfico correspondente e explique por que em uma frase, não force um gráfico vazio ou inventado.
...
2. `diagnostico-gargalos-{negocio}.html`: a calculadora interativa e os 3 gráficos do Passo 4 embutidos diretamente na página (não como anexo separado), mesmos tokens visuais do padrão da Aula 1 (fundo `#0A0A0A`, ouro `#C9B298`), com o resultado do gargalo e da receita em risco em destaque visual.
