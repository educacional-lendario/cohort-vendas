...
- No array `ENTREGAS` desse arquivo, encontre a linha com `id: 5` e troque `status: "pendente", html: null, md: null` por `status: "pronto", html: "playbook-objecoes-{negocio}.html", md: "playbook-objecoes-{negocio}.md"` (com o nome real do negócio no lugar de `{negocio}`). Não mude nenhuma outra linha do array — elas guardam o progresso das outras skills já entregues.
