...
**Gráfico de distribuição de temperatura, só a partir da Fase 3:** se o negócio já está na Fase 3 de rollout ou depois (Critérios 1, 3 e 4 pelo menos ativos, já existe alguma variação real de pontuação na base), inclua no HTML um gráfico de distribuição (donut ou pizza, SVG puro, sem biblioteca externa) contando quantos negócios estão Quente, Morno e Frio agora. Antes da Fase 3, não inclua esse gráfico, a base ainda é monocromática (quase tudo no mesmo nível), um gráfico de uma cor só não tem valor de leitura, apenas registre em texto que ele "ativa a partir da Fase 3" e por quê.

**Corte de temperatura em fase intermediária (Fase 3, antes do corte final de 80/40 fazer sentido):** o corte de 80/40 definido acima assume os 7 critérios ativos (soma máxima bem mais alta). Nas Fases 1 a 3, aplicar esse mesmo corte fixo deixaria a base inteira artificialmente fria. Em vez disso, calcule o corte proporcional à soma máxima possível com os critérios já ativos naquela fase: Quente é 70% ou mais da soma máxima da fase corrente, Morno é 30% a 69%, Frio é abaixo de 30%. Documente esse corte recalibrado no output (não é o 80/40 final, é o corte válido só pra fase atual), e recalibre de novo a cada fase nova que entrar em produção.
...
2. `lead-scoring-{negocio}.html`: mesma informação em página autocontida, mesmos tokens visuais do padrão da Aula 1 (fundo `#0A0A0A`, ouro `#C9B298`), com as 7 tabelas renderizadas como tabelas reais, a régua de temperatura em destaque visual (cores para Quente, Morno, Frio), o gráfico de distribuição de temperatura se a Fase 3 ou posterior já estiver ativa, e o passo a passo de implementação técnica também renderizado (não só no .md).
